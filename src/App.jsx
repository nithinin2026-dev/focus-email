    ...
    <div>
      ...
      <div style={{marginTop:36}}>
        ...
        <div style={{border:`2px solid ${T.bd}`,borderRadius:6,overflow:"hidden"}}>
          {slots.map(slot=>{const st=dayPlannerTasks.find(t=>t.time_slot===slot.key);const done=st&&!!st.completed_date;return(
            <div key={slot.key} style={{display:"flex",borderBottom:`1px solid ${T.bd}`,fontFamily:F,minHeight:42}}>
...
function AnalysisPage({sessions,setSessions}){
  ...
  const bCounts=buckets.map(b=>({...b,count:Object.values(dtAll).filter(m=>m>=b.min&&m<b.max).length}));const maxB=Math.max(...bCounts.map(b=>b.count),1);const dCols=["#E63946","#E63946","#F4A261","#2A9D8F","#2A9D8F","#457B9D"];
...
function SleepPage({sleepLogs,setSleepLogs}){
  ...
  const logMap={};sleepLogs.forEach(l=>{logMap[l.date]=l;});const barData=last14.map(d=>({date:d,mins:logMap[d]?.total_mins||0}));const maxSleep=Math.max(...barData.map(d=>d.mins),1);const bH=120;
  const r7=sleepLogs.slice(0,7);const avgSleep=r7.length>0?Math.round(r7.reduce((a,l)=>a+(l.total_mins||0),0)/r7.length):0;
  const avgBed=r7.length>0?r7.map(l=>l.sleep_start||"23:00").sort()[Math.floor(r7.length/2)]:"—";const avgWake=r7.length>0?r7.map(l=>l.wake_up||"07:00").sort()[Math.floor(r7.length/2)]:"—";
  const thisM=todayStr().slice(0,7);const mLogs=sleepLogs.filter(l=>l.date.startsWith(thisM));const mAvg=mLogs.length>0?Math.round(mLogs.reduce((a,l)=>a+(l.total_mins||0),0)/mLogs.length):0;
  ...
  return(
    <div>
      ...
      <div style={{fontSize:11,fontFamily:F,textTransform:"uppercase",letterSpacing:"0.15em",color:T.tx3,marginBottom:14,fontWeight:600,marginTop:32}}>Last 14 Days</div>
      <div style={{overflowX:"auto",paddingBottom:8}}><div style={{display:"flex",alignItems:"flex-end",gap:4,minWidth:14*36,height:bH+40,paddingTop:16}}>{barData.map(d=>{const h=d.mins>0?(d.mins/maxSleep)*bH:0;const c=d.mins>0?sleepColor(d.mins):T.bg3;const dl=new Date(d.date+"T12:00:00").getDate();return(<div key={d.date} style={{flex:1,minWidth:24,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",height:bH+40}}>{d.mins>0&&<span style={{fontSize:9,fontFamily:F,fontWeight:700,marginBottom:2,color:sleepColor(d.mins)}}>{formatHM(d.mins)}</span>}<div style={{width:"100%",height:h,background:c,borderRadius:"3px 3px 0 0",minHeight:d.mins>0?4:2}}/><span style={{fontSize:8,fontFamily:F,marginTop:3,color:T.tx3}}>{dl}</span></div>);})}</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:24}}>
        <div style={{background:T.bg3,padding:"16px",borderRadius:8,fontFamily:F}}><div style={{fontSize:10,color:T.tx3,textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:600,marginBottom:6}}>Weekly (last 7)</div><div style={{fontSize:22,fontWeight:700,color:T.tx}}>{formatHM(avgSleep)}</div><div style={{fontSize:11,color:T.tx2,marginTop:6}}>Bed: {avgBed}</div><div style={{fontSize:11,color:T.tx2}}>Wake: {avgWake}</div></div>
        <div style={{background:T.bg3,padding:"16px",borderRadius:8,fontFamily:F}}><div style={{fontSize:10,color:T.tx3,textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:600,marginBottom:6}}>Monthly</div><div style={{fontSize:22,fontWeight:700,color:T.tx}}>{formatHM(mAvg)}</div><div style={{fontSize:11,color:T.tx2,marginTop:6}}>{mLogs.length} nights logged</div></div>
      </div>
...
export default function App(){
  ...
  useEffect(()=>{document.body.style.background=theme.bg;document.documentElement.style.background=theme.bg;},[isDark]);
