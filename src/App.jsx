      ...
      <div style={{padding:"24px 20px 16px",borderBottom:`1px solid ${T.bd}`}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <div style={{flex:1,height:4,background:T.bd2,borderRadius:2,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${Math.round((doneCount/items.length)*100)}%`,background:doneCount===items.length?"#2A9D8F":"#F4A261",borderRadius:2,transition:"width 0.4s"}}/>
          </div>
          <span style={{fontSize:10,fontWeight:700,color:doneCount===items.length?"#2A9D8F":"#F4A261"}}>{doneCount}/{items.length}</span>
        </div>
      </div>
...
function TrackerSection({trackerLogs,setTrackerLogs}){
  ...
  return(<div style={{background:T.bg2,borderRadius:10,padding:mob?14:18,border:`1px solid ${T.bd}`,fontFamily:F}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10,flexWrap:"wrap",gap:8}}><div style={{fontSize:11,textTransform:"uppercase",letterSpacing:"0.15em",color:T.tx3,fontWeight:600}}>Daily Checklist</div><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14,fontWeight:800,color:T.tx}}>{tabStreak>0?`${tabStreak} 🔥`:"0"}</span><span style={{fontSize:10,color:T.tx3}}>streak</span><span style={{fontSize:12,fontWeight:700,color:doneTodayCount===habits.length?"#2A9D8F":T.tx3}}>{doneTodayCount}/{habits.length}</span></div></div><div style={{height:3,background:T.bg3,borderRadius:2,overflow:"hidden",marginBottom:12}}><div style={{height:"100%",width:`${pct}%`,background:pct===100?"#2A9D8F":"#F4A261",borderRadius:2,transition:"width 0.4s"}}/></div><div style={{display:"flex",background:T.bg3,borderRadius:6,overflow:"hidden",marginBottom:12,border:`1px solid ${T.bd}`}}>{tabs.map(tab=>(<button key={tab} onClick={()=>setActiveTab(tab)} style={{flex:1,padding:"8px 6px",border:"none",background:activeTab===tab?T.btn:"transparent",color:activeTab===tab?T.btnT:T.tx3,fontFamily:F,fontSize:11,fontWeight:700,cursor:"pointer"}}>{tabLabels[tab]}</button>))}</div>{activeTab==="cat"&&renderHabits("cat",TRACKER_TABS.cat)}{activeTab==="personal"&&(<><div style={{fontSize:9,color:T.tx3,letterSpacing:3,textTransform:"uppercase",marginBottom:6,fontWeight:600}}>☀️ Morning</div>{renderHabits("personal",TRACKER_TABS.personal.morning)}<div style={{fontSize:9,color:T.tx3,letterSpacing:3,textTransform:"uppercase",margin:"10px 0 6px",fontWeight:600}}>🌙 Evening</div>{renderHabits("personal",TRACKER_TABS.personal.evening)}</>)}</div>);
}
  ...
  return(
    <div style={{fontFamily:F}}>
      ...
      <div style={{marginBottom:8}}>
        <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:"0.15em",color:T.tx3,marginBottom:12,fontWeight:600}}>Log Study Time</div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <input value={mTag} onChange={e=>setMTag(e.target.value)} placeholder="Tag" style={{...iStyle,flex:1,minWidth:100}} onKeyDown={e=>e.key==="Enter"&&logManual()}/>
          <input value={mMins} onChange={e=>setMMins(e.target.value)} placeholder="mins" type="number" style={{...iStyle,width:80}} onKeyDown={e=>e.key==="Enter"&&logManual()}/>
      ...
      <div style={{marginBottom:32}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:10,marginTop:16}}>
          <span style={{fontSize:11,textTransform:"uppercase",letterSpacing:"0.15em",color:T.tx3,fontWeight:600}}>Today's Sessions</span>
          <span style={{fontSize:14,fontWeight:700,color:T.tx}}>{formatHM(tTotal)} {tTotal>=120&&"🔥"}</span>
        </div>
        {tSess.length===0&&(<div style={{color:T.tx4,fontSize:13,padding:"12px 0",textAlign:"center"}}>No sessions yet today</div>)}
        {tSess.map(s=>(<div key={s.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.bd}`,fontSize:14}}>
          {editSessId===s.id?(<input autoFocus value={editSessTag} onChange={e=>setEditSessTag(e.target.value)} onBlur={()=>renameSession(s,editSessTag)} onKeyDown={e=>{if(e.key==="Enter")renameSession(s,editSessTag);if(e.key==="Escape")setEditSessId(null);}} style={{border:"none",borderBottom:`2px solid ${T.bd3}`,background:"transparent",fontSize:14,fontFamily:F,fontWeight:600,outline:"none",color:T.tx,padding:"2px 0",flex:1,marginRight:8}}/>):(<span onClick={()=>{setEditSessId(s.id);setEditSessTag(s.tag);}} style={{fontWeight:600,color:T.tx,cursor:"pointer"}}>{s.tag}</span>)}
          <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{color:T.tx3}}>{formatHM(s.duration)}</span><button onClick={async()=>{await deleteSession(s.id);setSessions(p=>p.filter(x=>x.id!==s.id));}} style={{border:"none",background:"none",cursor:"pointer",color:T.tx4,fontSize:16,padding:"0 2px"}}>✕</button></div>
        </div>))}
      </div>
      <div style={{borderTop:`1px solid ${T.bd}`,marginBottom:28}}/>
      ...
      {goals.length===0&&!showAdd&&(<div style={{textAlign:"center",padding:"40px 20px",color:T.tx4,fontSize:14}}><div style={{fontSize:48,marginBottom:16}}>🎯</div><div style={{fontWeight:700,color:T.tx3,marginBottom:8}}>No goals yet</div><div>Create your first goal to start tracking progress</div></div>)}
      {goal&&S&&(<div>
          ...
          return(<>
            ...
            <div style={{position:"relative"}}>
              <div style={{display:"flex",alignItems:"flex-end",gap:mob?6:10,height:140,paddingTop:16,marginBottom:8}}>
                {S.wData.map((d,i)=>{const isFut=d.date>todayStr();const proj=projMins[d.date]||0;const actualH=d.mins>0?Math.max((d.mins/scaleMax)*100,8):isFut?0:4;const projH=proj>0?Math.max((proj/scaleMax)*100,4):0;const totalMins=d.mins+proj;let bColor=isFut&&!proj?T.bd:totalMins>=dynamicDaily?"#2A9D8F":totalMins>=dynamicDaily*0.8?"#F4A261":totalMins>0?"#E63946":T.bd2;
      ...
      <div style={{borderTop:`1px solid ${T.bd}`,paddingTop:32,marginTop:8,display:"flex",justifyContent:"center",paddingBottom:8}}>
...
function HabitsPage({habits,setHabits,habitLogs,setHabitLogs}){
  ...
  const addHabit=async()=>{const days=parseInt(nDays);if(!nName.trim()||isNaN(days)||days<=0||!nStart)return;const sv=await insertHabit(nName.trim(),nIcon,days,nStart);if(sv){setHabits(p=>[...p,sv]);setShowAdd(false);setNName("");setNIcon("🎯");setNDays("");setNStart(todayStr());}};
  ...
  const getHabitStats=(habit)=>{const logs=habitLogs.filter(l=>l.habit_id===habit.id);const logDates=new Set(logs.map(l=>l.date));const today=todayStr();const start=new Date(habit.start_date+"T12:00:00");const todayDate=new Date(today+"T12:00:00");const dayNum=Math.max(1,Math.floor((todayDate-start)/86400000)+1);const doneToday=logDates.has(today);let streak=0;const d=new Date(todayDate);if(doneToday){streak=1;d.setDate(d.getDate()-1);}else{d.setDate(d.getDate()-1);}while(d>=start){if(logDates.has(dateToStr(d))){streak++;d.setDate(d.getDate()-1);}else break;}let best=0,cur=0;const allDates=[...logDates].sort();for(let i=0;i<allDates.length;i++){if(i===0){cur=1;}else{const prev=new Date(allDates[i-1]+"T12:00:00");const curD=new Date(allDates[i]+"T12:00:00");if((curD-prev)===86400000)cur++;else cur=1;}if(cur>best)best=cur;}return{dayNum,totalDone:logs.length,streak,best,doneToday,logDates};};
...
function GymPage({gymLogs,setGymLogs}){
  ...
  const gc=mob?"70px 1fr 50px":"90px 1fr 60px";
  return(
    <div style={{fontFamily:F}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:gc,gap:0,borderBottom:`2px solid ${T.bd3}`,paddingBottom:8,marginBottom:4}}>
        ...
        <span style={{fontSize:10,fontWeight:700,color:T.tx3,textTransform:"uppercase",letterSpacing:"0.1em"}}>Exercises / Note</span>
        <span style={{fontSize:10,fontWeight:700,color:T.tx3,textTransform:"uppercase",letterSpacing:"0.1em",textAlign:"right"}}>Sets</span>
      </div>
      {allDates.map(date=>{
          ...
          <div key={date} style={{marginLeft:-8,marginRight:-8,paddingLeft:8,paddingRight:8,borderRadius:2,background:rBg,borderBottom:`1px solid ${rBd}`}}>
            {isE?(
              <div style={{padding:"12px 0"}}>
                <div style={{display:"grid",gridTemplateColumns:gc,gap:0,alignItems:"start",marginBottom:10}}>
                  ...
                  <div style={{paddingRight:8}}>
                    {exercises.map((ex,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span style={{flex:1,fontSize:13,fontWeight:600,color:T.tx}}>{ex}</span><button onClick={()=>setExercises(p=>p.filter((_,j)=>j!==i))} style={{border:"none",background:"none",cursor:"pointer",color:T.tx4,fontSize:14,padding:0}}>✕</button></div>))}
                    <div style={{display:"flex",gap:6,marginTop:6}}>
                      ...
                      <button onClick={addExercise} style={{border:`2px solid ${T.bd3}`,background:T.btn,color:T.btnT,padding:"3px 10px",fontSize:11,fontFamily:F,fontWeight:700,cursor:"pointer"}}>+</button>
                    </div>
                    <input value={note} onChange={e=>setNote(e.target.value)} placeholder="Session note (energy, how it felt...)" onKeyDown={e=>e.key==="Enter"&&saveLog(date)} style={{marginTop:8,width:"100%",border:"none",borderBottom:`1px solid ${T.bd2}`,background:"transparent",fontSize:12,fontFamily:F,outline:"none",color:T.tx2,padding:"3px 0",boxSizing:"border-box"}}/>
                  </div>
                  <div style={{textAlign:"right",paddingTop:2}}><span style={{fontSize:13,fontWeight:700,color:"#2A9D8F"}}>{exercises.length}</span></div>
                </div>
                <div style={{display:"flex",gap:8,paddingTop:4}}>
                  <button onClick={()=>saveLog(date)} disabled={saving} style={{border:`2px solid ${T.bd3}`,background:T.btn,color:T.btnT,padding:"5px 14px",fontSize:11,fontFamily:F,fontWeight:700,cursor:"pointer"}}>Save</button>
              ...
              <div onClick={()=>startEdit(date)} style={{display:"grid",gridTemplateColumns:gc,gap:0,padding:"10px 0",cursor:"pointer",alignItems:"center"}}>
                <div style={{display:"flex",flexDirection:"column"}}><span style={{fontWeight:700,fontSize:12,color:T.tx}}>{dl}</span><span style={{fontSize:10,color:T.tx3}}>{dLabel}</span></div>
                <div style={{paddingRight:8,minWidth:0}}>
                  {hasLog?(<><div style={{fontSize:13,fontWeight:600,color:T.tx,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{log.exercises.slice(0,2).join(" · ")}{log.exercises.length>2?` +${log.exercises.length-2}`:""}</div>{log.note&&<div style={{fontSize:11,color:T.tx3,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{log.note}</div>}</>):(<span style={{color:T.tx4,fontSize:13}}>{isT?"Tap to log today's workout":"—"}</span>)}
                </div>
                <div style={{textAlign:"right"}}>{hasLog&&<span style={{fontSize:13,fontWeight:700,color:"#2A9D8F"}}>{log.exercises.length}</span>}</div>
              </div>
            )}
          </div>
        );
      })}
      {allDates.length===0&&(<div style={{textAlign:"center",padding:"60px 20px",color:T.tx4,fontSize:14}}><div style={{fontSize:48,marginBottom:16}}>🏋️</div><div style={{fontWeight:700,color:T.tx3,marginBottom:8}}>No gym logs yet</div><div>Tap today's row to log your first workout</div></div>)}
      <div style={{display:"flex",gap:16,marginTop:20,fontSize:10,color:T.tx3,flexWrap:"wrap"}}>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,background:T.rG,border:`1px solid ${T.rGB}`,display:"inline-block",borderRadius:2}}/> Logged</span>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,background:T.rR,border:`1px solid ${T.rRB}`,display:"inline-block",borderRadius:2}}/> Today</span>
...
const SPEND_CATS=[{key:"need",emoji:"🟢"},{key:"want",emoji:"🟡"},{key:"waste",emoji:"🔴"}];
...
function SpendingPage(){
  ...
  const[date,setDate]=useState(todayStr());const[amount,setAmount]=useState("");const[label,setLabel]=useState("");const[category,setCategory]=useState("need");const[saving,setSaving]=useState(false);
  const[expandedMonths,setExpandedMonths]=useState({});
  useEffect(()=>{loadSpending().then(d=>{setEntries(d);setLoaded(true);});},[]);
  const addEntry=async()=>{const amt=parseFloat(amount);if(!date||isNaN(amt)||amt<=0||!label.trim())return;setSaving(true);const sv=await insertSpending(date,amt,label.trim(),category);if(sv){setEntries(p=>[sv,...p].sort((a,b)=>b.date.localeCompare(a.date)||b.amount-a.amount));}setAmount("");setLabel("");setDate(todayStr());setCategory("need");setSaving(false);};
  const monthMap={};entries.forEach(e=>{const mk=e.date.slice(0,7);if(!monthMap[mk])monthMap[mk]=[];monthMap[mk].push(e);});
  ...
  const monthCatTotals=(mk)=>{const t={need:0,want:0,waste:0};monthMap[mk].forEach(e=>{t[e.category]=(t[e.category]||0)+Number(e.amount);});return t;};
  const toggleMonth=(mk)=>{setExpandedMonths(p=>({...p,[mk]:!p[mk]}));};
  const gc=mob?"62px 1fr 60px 22px":"80px 1fr 70px 24px";
    ...
    <div style={{fontFamily:F}}>
      ...
      <div style={{background:T.bg2,borderRadius:10,padding:mob?14:18,border:`1px solid ${T.bd}`,marginBottom:28}}>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
          <input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{...iStyle,flex:mob?"1 1 100%":"0 0 auto"}}/>
          <input value={amount} onChange={e=>setAmount(e.target.value)} placeholder="₹ Amount" type="number" step="0.01" onKeyDown={e=>e.key==="Enter"&&addEntry()} style={{...iStyle,flex:1,minWidth:80}}/>
          <input value={label} onChange={e=>setLabel(e.target.value)} placeholder="What for?" onKeyDown={e=>e.key==="Enter"&&addEntry()} style={{...iStyle,flex:"1 1 100%"}}/>
        </div>
        <div style={{display:"flex",gap:6,marginBottom:10}}>
          {SPEND_CATS.map(c=>(<button key={c.key} onClick={()=>setCategory(c.key)} style={{padding:"6px 12px",border:`2px solid ${category===c.key?catColor(c.key):T.bd2}`,background:category===c.key?catColor(c.key)+"18":"transparent",color:category===c.key?catColor(c.key):T.tx3,fontFamily:F,fontSize:13,fontWeight:700,cursor:"pointer",borderRadius:6}}>{c.emoji}</button>))}
        </div>
        <button onClick={addEntry} disabled={saving} style={{padding:"9px 24px",border:`2px solid ${T.bd3}`,background:T.btn,color:T.btnT,fontSize:12,fontFamily:F,fontWeight:700,cursor:saving?"default":"pointer",letterSpacing:"0.06em",textTransform:"uppercase",opacity:saving?0.5:1}}>{saving?"...":"+ Add"}</button>
      </div>
      {entries.length===0&&(<div style={{textAlign:"center",padding:"60px 20px",color:T.tx4,fontSize:14}}><div style={{fontSize:48,marginBottom:16}}>💸</div><div style={{fontWeight:700,color:T.tx3,marginBottom:8}}>No entries yet</div><div>Log your first spend above</div></div>)}
      {sortedMonths.map(mk=>{
        const isCurrent=mk===currentMonth;const isExpanded=isCurrent?true:!!expandedMonths[mk];const total=monthTotal(mk);const cats=monthCatTotals(mk);const rows=sortedEntries(mk);
        return(<div key={mk} style={{marginBottom:16}}>
          <div onClick={isCurrent?undefined:()=>toggleMonth(mk)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:T.bg2,borderRadius:isExpanded?"10px 10px 0 0":10,border:`1px solid ${T.bd}`,cursor:isCurrent?"default":"pointer",userSelect:"none"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",justifyContent:"flex-end"}}>
              {SPEND_CATS.map(c=>cats[c.key]>0&&(<span key={c.key} style={{fontSize:12,fontWeight:700,color:catColor(c.key)}}>{c.emoji} ₹{Math.round(cats[c.key]).toLocaleString("en-IN")}</span>))}
              <span style={{fontSize:14,fontWeight:800,color:T.tx}}>₹{Math.round(total).toLocaleString("en-IN")}</span>
            </div>
          </div>
          {isExpanded&&(<div style={{border:`1px solid ${T.bd}`,borderTop:"none",borderRadius:"0 0 10px 10px",overflow:"hidden"}}>
            ...
            {rows.map((e,idx)=>{const dl=new Date(e.date+"T12:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric"});const isLast=idx===rows.length-1;return(<div key={e.id} style={{display:"grid",gridTemplateColumns:gc,gap:0,padding:"10px 12px",borderBottom:isLast?"none":`1px solid ${T.bd}`,background:idx%2===0?T.bg:T.bg2,alignItems:"center"}}><span style={{fontSize:13,fontWeight:600,color:T.tx3,whiteSpace:"nowrap"}}>{dl}</span><span style={{fontSize:14,fontWeight:600,color:T.tx,paddingRight:8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.label}</span><span style={{fontSize:14,fontWeight:700,color:catColor(e.category),textAlign:"right",whiteSpace:"nowrap"}}>₹{Number(e.amount).toLocaleString("en-IN",{maximumFractionDigits:0})}</span><span style={{textAlign:"center",fontSize:14}}>{catEmoji(e.category)}</span></div>);})}
          </div>)}
        </div>);
      })}
    </div>
...
function SleepPage({sleepLogs,setSleepLogs}){
  ...
  const mAvgBed=avgTime(mLogs.map(l=>l.sleep_start).filter(Boolean));const mAvgWake=avgTime(mLogs.map(l=>l.wake_up).filter(Boolean));
  ...
  return(
    <div>
      ...
      {sleepLogs.map(l=>{const c=sleepColor(l.total_mins||0);const dl=new Date(l.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"});return(<div key={l.id} style={{display:"grid",gridTemplateColumns:mob?"1fr 60px 60px 60px":"90px 70px 70px 70px",padding:"8px 0",borderBottom:`1px solid ${T.bd}`,fontFamily:F,fontSize:mob?12:13}}><span style={{fontWeight:600,fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:T.tx}}>{dl}</span><span style={{color:T.tx2}}>{l.sleep_start||"—"}</span><span style={{color:T.tx2}}>{l.wake_up||"—"}</span><span style={{textAlign:"right",fontWeight:700,color:c}}>{l.total_mins?formatHM(l.total_mins):"—"}</span></div>);})}
    </div>
  );
}
